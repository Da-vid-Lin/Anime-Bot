...
class CommandHandler {
  ...
  handle(message) {
    ...
    if ((command.name == 'sekki' && !this.isOwner(message.member))) {
  ...
  isAdmiral(member) {
    return member.roles.cache.find(r => r.id == this.discord.app.config.discord.admiralRole)
  }
  ...
  isOwners(member) {
    return member.id == this.discord.app.config.discord.ownersId
  }                                  
}

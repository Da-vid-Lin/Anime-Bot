...
class AnimePicturesCommand extends DiscordCommand {
  constructor(discord) {
    super(discord)

    this.name = 'a-pictures'
    this.aliases = ['apict']
    this.description = 'Sends the anime pictures of given anime'
  }
  ...
  onCommand(message) {
    ...
    if (anime == undefined){message.channel.send(`Usage: !a-pictures [name]`)}
    else{
      if (anime != undefined){
      ...
      else{message.channel.send(`Usage: !a-pictures [name]`)}
    }
    
  }
}

module.exports = AnimePicturesCommand
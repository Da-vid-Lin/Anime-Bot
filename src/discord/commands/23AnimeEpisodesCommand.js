...
class AnimeEpisodesCommand extends DiscordCommand {
  constructor(discord) {
    super(discord)

    this.name = 'a-episodes'
    this.aliases = ['aepis']
    this.description = 'Sends the anime episodes of given anime'
  }
  ...
  onCommand(message) {
    ...
    if (anime == undefined){message.channel.send(`Usage: !a-episodes [name]`)}
    else{
      if (anime != undefined){
        ...
        .then(({ data }) => {
          if (data.length >= 1){
            ...
            .then(({ data }) => {
              ...
              for (let x = 0; x < 25 && x < data.length; x++){
                Data.push(`${data[x].mal_id}: ${data[x].title}`)
              }
              ...
              message.channel.send({
                embed: {
                  ...
                  description: "",
                  fields: {name: 'Episodes',value: Data.join('\n')},
                  image: {url: animepng},
                  url: animeurl,
      ...
      else{message.channel.send(`Usage: !a-episodes [name]`)}
    }
    
  }
}

module.exports = AnimeEpisodesCommand
...
class MangaSimilarCommand extends DiscordCommand {
  constructor(discord) {
    super(discord)

    this.name = 'm-similar'
    this.aliases = ['msimi']
    this.description = 'Sends the manga similar of given manga'
  }
  ...
  onCommand(message) {
    ...
    if (manga == undefined){message.channel.send(`Usage: !m-similar [name]`)}
    else{
      if (manga != undefined){
      ...
      else{message.channel.send(`Usage: !m-similar [name]`)}
    }
    
  }
}

module.exports = MangaSimilarCommand